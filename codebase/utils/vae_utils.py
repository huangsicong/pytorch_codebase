...
def prepare_vae(writer, hparams):
  ...
  if hparams.train_first:
    ...
    if hparams.resume_training:
      try:
        ...
        note_taking("re-initializing... will train from scracth instead, in: {}"
      ...
      hparams.step = 0
      test_loss_list = list()
    model = train_and_test(test_loss_list, model, optimizer, writer, hparams)
  ...
  if hparams.overwrite_variance is not None:
    model.x_logvar = nn.Parameter(
        torch.log(
            torch.tensor(hparams.overwrite_variance,
                         dtype=hparams.tensor_type)),
        requires_grad=False)
    note_taking(
        "NOTICE! model.x_logvar is overwrited: {}".format(model.x_logvar))
  return model
